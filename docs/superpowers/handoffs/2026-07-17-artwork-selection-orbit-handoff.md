...
- 如需完整撤销功能实现，使用 `git revert` 针对功能/测试链创建反向提交，且必须严格按逆序执行：`b968239` → `55cf226` → `1de00f6` → `aaca032` → `6ed931b`。这些 revert 仅处理功能/测试链；规格、计划和交接文档提交是否保留，需单独决定。
- 不重写历史，不使用 reset、rebase 或强制 push 作为回退方式。
- 每一步 revert 如发生冲突，立即停止审查冲突原因，不盲目选择任一版本；完整回退后运行 `npm test` 和 `npm run build:showcase`，并在 WebGL 浏览器复查交互。
