export async function uploadDesignAsset({ atlas, design, metadata, turnstileToken, endpoint = '/api/design-assets' }) {
  if (!(atlas instanceof Blob) || atlas.type !== 'image/png') throw new Error('A PNG UV atlas is required.');
  if (typeof turnstileToken !== 'string' || turnstileToken.length === 0) throw new Error('Turnstile verification is required.');
