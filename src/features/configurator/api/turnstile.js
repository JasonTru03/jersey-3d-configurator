export async function getDesignUploadTurnstileToken({ endpoint = '/api/design-assets/config', turnstile = window.turnstile } = {}) {
  ...
  const token = await turnstile.execute(config.turnstileSiteKey, { action: 'design_upload' });
